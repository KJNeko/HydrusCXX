...
HydrusCXX::HydrusCXX( const std::filesystem::path& path )
	: client( path.string() + "client.db" ), master( path.string() + "client.master.db" ), mappings( path.string() + "client.Mappings.db" )
{
}
...
uint Master::getIDFromSHA256( std::bitset<SHA512LENGTH> hash )
{
	fixme return 0;
}
...
std::bitset<MD5LENGTH> Master::getMD5( uint id )
{
	...
	db << "select md5 from local_hashes where hash_id == " + std::to_string( id ) >> [ & ]( std::vector<unsigned char> blob ) {
		memcpy( &hash, blob.data(), blob.size() );
	};
...
std::bitset<SHA1LENGTH> Master::getSHA1( uint id )
{
	...
	db << "select sha1 from local_hashes where hash_id == " + std::to_string( id ) >> [ & ]( std::vector<unsigned char> blob ) {
		memcpy( &hash, blob.data(), blob.size() );
	};
...
std::bitset<SHA512LENGTH> Master::getSHA512( uint id )
{
	...
	db << "select sha512 from local_hashes where hash_id == " + std::to_string( id ) >> [ & ]( std::vector<unsigned char> blob ) {
		memcpy( &hash, blob.data(), blob.size() );
	};
...
{
	uint temp;
	db << "select tag_id from tags where subtag_id == " + std::to_string( subtag_id ) + " and namespace_id == " + std::to_string( group_id ) >>
		[ & ]( uint tag_id ) {
			temp = tag_id;
		};
	return temp;
...
std::pair<uint, uint> Master::getTagPair( uint tag_id )
{
	...
	db << "select namespace_id, subtag_id from tags where tag_id == " + std::to_string( tag_id ) >> [ & ]( uint group, uint subtag ) {
...
std::pair<std::string, std::string> Master::getTagPairString( uint tag_id )
{
	...
	db << "select namespace_id, subtag_id from tags where tag_id == " + std::to_string( tag_id ) >> [ & ]( uint namespace_id, uint subtag_id ) {
